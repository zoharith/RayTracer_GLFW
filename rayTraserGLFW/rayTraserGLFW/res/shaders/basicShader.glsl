...
bool shdow_spot(vec3 point,vec3 direction1,int j,float len)
{
 float dist=-1;
 for(int i=0;i<sizes[0];i++){
  ...
  if(dist>0.0001){
    float len1=length(dist*normalize(direction1));//to object
      if(len1<len){
        return true;
       }
  }
...
bool get_shadow(vec3 point,vec3 direction1,int j)
{
 float dist=-1;
 for(int i=0;i<sizes[0];i++){
  if (objects[i].w>0){
...
vec3 calclighting(vec3 point,vec3 direction1,float minvalue,float diffusebla,bool _mirror)
{
...
for(int i=0;i<sizes[1];i++)
    ...
    bool shadow=get_shadow(point,-lightsDirection[i].xyz,i);
  if(_mirror||shadow==false){
    ...
    color+=clamp(specular.xyz*getspecular(normal, direction1,i, minindex,lightsDirection[i].xyz),0.0f,1.0f);
      ...
      float len=length(point-lightPosition[i].xyz);
      bool shadow=shdow_spot(point,-angle,i,len);
      if(_mirror||shadow==false){
...
vec3 colorCalc( vec3 intersectionPoint)
{
  ...
  if(minvalue<0){
   ...
   diffusebla=diffuseblack(point,minindex);
    ...
    color+=calclighting(point,direction1,minvalue,diffusebla,false);
...
if(mirrorbool&& minindex==mirrorindex){
  c=vec4(0,0,0,0);
   ...
   vec3 mirrordirection=normalize(reflection_vec(mirrornormal,angle));
     minindex=-1;
     minvalue=-1;
     bool mirrorshadowbool=true;
     if(eye.w==2.0f)
     mirrorshadowbool=true;
     if(eye.w==3.0f)
     mirrorshadowbool=false;
   ...
   c=vec4(0.1,0.2,0.3,1)*objColors[minindex];
      c+=vec4(clamp(calclighting(point,mirrordirection,minvalue,1,mirrorshadowbool),0,1),1);    
}
  gl_FragColor =c;
}
